 ...
 * @param {string} name - prenom ou nom complet
 * @param {Array} items - liste des produits dans le panier
 * @param {number} totalCents - montant total en centimes
...
export async function sendAbandonedCartEmail(to, name, items, totalCents) {
  ...
  // Limiter a 4 produits dans l'email
  ...
  const extraHtml = extraCount > 0
    ? `<tr><td style="padding:12px 0; color:#888; font-style:italic;">+ ${extraCount} autre(s) produit(s) dans votre panier</td></tr>`
    : "";
    ...
    <div style="font-family: system-ui, -apple-system, sans-serif; max-width: 600px; margin: 0 auto; background: #fff;">
      ...
      <div style="padding: 32px 24px;">
        <p style="color: #241712; font-size: 16px; line-height: 1.6;">
          Vos produits vous attendent ! Ne les laissez pas s'echapper.
        </p>
        <table style="width: 100%; margin: 24px 0;">
          ${itemsHtml}
          ${extraHtml}
          <tr>
            <td style="padding: 16px 0; font-size: 18px; font-weight: 700; color: #241712;">
              Total : ${totalFormatted} F CFA
            </td>
          </tr>
        </table>
        <a href="${BASE_URL}/cart" style="display: block; background: #e6a623; color: #241712; text-decoration: none; padding: 16px; border-radius: 8px; text-align: center; font-weight: 600; font-size: 16px; margin: 24px 0;">
          Retour a mon panier →
        </a>
        <p style="color: #888; font-size: 13px; line-height: 1.5; margin-top: 32px; padding-top: 24px; border-top: 1px solid #eee;">
          Vous recevez cet email car vous avez ajoute des produits a votre panier sur Kimoxa.<br>
          Si vous ne souhaitez plus recevoir ces relances, vous pouvez simplement ignorer cet email.
        </p>
      </div>
    </div>
  `;
