...
export async function rateLimit(key, options = {}) {
  // E2E (local + CI) : rate limits desactives quand ALLOW_TEST_HELPERS=1
  // (environnement de test uniquement ; production reste protegee)
  if (process.env.ALLOW_TEST_HELPERS === "1" && !key.includes("login")) return true;
