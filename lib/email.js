...
// URL de base utilisée dans les emails transactionnels.
const BASE_URL =
  process.env.NEXT_PUBLIC_APP_URL || "https://kimoxa.com";

/**
 * Envoie un email via Resend.
 *
 * En E2E, lorsque E2E_TEST_MODE=1, aucun appel réseau Resend
 * n'est effectué. L'envoi est simulé avec un résultat réussi.
 *
 * @param {object} params
...
export async function sendMail({
  ...
  const from =
    ...
    "Kimoxa <onboarding@resend.dev>";

  // ---------------------------------------------------------
  // 1) Pas de clé Resend
  // ---------------------------------------------------------
  //
  // On conserve le comportement gracieux existant.
  if (!apiKey) {
    console.warn(
      "[email] RESEND_API_KEY non défini — email non envoyé"
    );
  ...
  // ---------------------------------------------------------
  // 2) Override destinataire de test
  // ---------------------------------------------------------
  //
  // Utilisé uniquement lorsque l'option est explicitement activée.
  if (process.env.EMAIL_TEST_OVERRIDE === "true") {
    const testEmail =
      process.env.EMAIL_TEST_RECIPIENT ||
      "fasoshop.marketplace@gmail.com";

    console.log(
      `[email] Override dev : ${to} → ${testEmail}`
    );
  ...
  //
  // IMPORTANT :
  // - le code métier continue son exécution ;
  // - aucune requête externe Resend n'est effectuée ;
  // - la production n'est pas affectée.
  if (process.env.E2E_TEST_MODE === "1") {
    console.log(
      "[email] E2E_TEST_MODE=1 — envoi Resend simulé",
      {
        to,
        subject,
      }
...
/**
 * Envoie un email stock bas UNIQUEMENT au franchissement
 * du seuil.
 *
 * Anti-spam :
 * old > threshold ET new <= threshold.
 */
...
/**
 * Envoie une notification nouvelle commande au vendeur.
 */
export async function sendNewOrderToVendor({
  ...
  const tpl =
    emailTemplates.newOrderToVendor({
      vendorName:
...
export const emailTemplates = {
      ...
      html: `
        <div style="font-family: system-ui, sans-serif; max-width: 600px; margin: 0 auto; padding: 20px;">
          ...
          <p style="margin-top: 30px;">
            <a
              href="${BASE_URL}/vendor/dashboard"
              style="
                background: #d4af37;
                color: #000;
                padding: 12px 24px;
      ...
      html: `
        <div style="font-family: system-ui, sans-serif; max-width: 600px; margin: 0 auto; padding: 20px;">
          ...
          <p>
            Votre boutique
            <strong>${shopName}</strong>
            n'a pas pu être validée pour le moment.
          </p>
          ...
          <p style="margin-top: 20px;">
            Vous pouvez corriger ces éléments
            et soumettre à nouveau votre boutique.
          </p>
    ...
    return {
      ...
      html: `
        <div style="font-family: system-ui, sans-serif; max-width: 600px; margin: 0 auto; padding: 20px;">
          ...
          <p>
            Votre paiement Mobile Money de
        ...
        ">
          ...
          ">
            ...
            <p>
              ...
              est passé sous le seuil d'alerte.
            </p>
            ...
            <p>
              Pensez à réapprovisionner pour éviter
              une rupture et des ventes perdues.
            </p>
          ...
          <p style="
            ...
            text-align: center;
          ">
            Kimoxa — La marketplace des commerçants
            du Burkina Faso
          </p>
        </div>
      `,
  ...
  /**
   * Notification nouvelle commande au vendeur.
   */
  newOrderToVendor({
    vendorName,
    shopName,
        ...
        (i) => `
          <tr>
            ...
            <td style="
              ...
              text-align: right;
            ">
              ${i.price.toLocaleString("fr-FR")} FCFA
            </td>
            ...
            <td style="
              ...
              font-weight: 600;
            ">
              ${i.lineTotal.toLocaleString("fr-FR")} FCFA
            </td>
          </tr>
        `
      )
