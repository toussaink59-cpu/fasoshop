  ...
  try {
    ...
    if (
      ...
      return {
       ...
       // -------------------------------------------------
    // Échec EXPLICITE uniquement (seul cas retryable)
    // -------------------------------------------------
    if (FINAL_FAILURE_STATUSES.has(status)) {
      ...
      await markFailed(idempotencyKey, providerMessage);
    ...
    // -------------------------------------------------
    // Tout le reste (200 inconnu, 4xx/5xx ambigus) = INCERTAIN
    // -------------------------------------------------
    await markUnconfirmed(
      idempotencyKey,
      `Réponse ambiguë (${response.status}, statut: ${status || "absent"}).`
    );

    return {
      ok: false,
      status: "unconfirmed",
      error: "Réponse fournisseur ambiguë. Vérifiez le statut avant toute nouvelle tentative.",
    };
  } catch (error) {
    /*
     * TRÈS IMPORTANT :
