...
export async function getProducts(
  ...
  const [countResult] = await sql`
    ...
    WHERE p.status = 'active'
      ...
      AND (${q}::text IS NULL OR p.search_vector @@ plainto_tsquery('french', ${q}))
  ...
  const products = await sql`
    SELECT p.id, p.name, p.description, p.price, p.compare_at_price, p.stock_quantity, p.images, p.sku,
           CASE WHEN ${q}::text IS NOT NULL THEN ts_rank(p.search_vector, plainto_tsquery('french', ${q})) ELSE 0 END AS relevance_score, p.images, p.condition,
    ...
    WHERE p.status = 'active'
      ...
      AND (${city}::text IS NULL OR s.city = ${city})
      AND (${minRating}::numeric IS NULL OR COALESCE(r.avg_rating, 0) >= ${minRating}::numeric)
    ORDER BY (p.is_sponsored AND p.sponsored_until > NOW()) DESC, CASE WHEN ${q}::text IS NOT NULL THEN ts_rank(p.search_vector, plainto_tsquery('french', ${q})) ELSE 0 END DESC, ${SORT_CLAUSES[sortKey](sql)}, p.id DESC
    LIMIT ${safeLimit + 1}
  `;
